...
function Bot(options) {
  ...
  /**
   * Set custom category for logging traffic
   */
  logger.loggers.add('messagelogger', {
          file: {
...
Bot.prototype.sendToIRC = function(message) {
  ...
  if (ircChannel) {
    ...
    var mlog = logger.loggers.get('messagelogger');
    mlog.remove(logger.transports.Console);                                                                                                                                                                                                    
    mlog.info('TO_IRC', ircChannel, user.name, text);
...
Bot.prototype.sendToSlack = function(author, channel, text) {
  ...
  if (slackChannelName) {
    ...
    var mlog = logger.loggers.get('messagelogger');
    mlog.remove(logger.transports.Console);                                                                                                                                                                                                    
    mlog.info('TO_SLACK', channel, author, text);
